...
#if 0
template<>
ExtMove* generate<CHECK>(const Position& pos, ExtMove* mlist)
{
...
ExtMove* generate<QUIET_CHECKS>(const Position& pos, ExtMove* mlist)
{
	...
	bool bUchifudume = false;
	mlist = (us == BLACK) ? last=pos.generate_check<BLACK>(mlist, bUchifudume)
		: last=pos.generate_check<WHITE>(mlist, bUchifudume);
...
ExtMove* generate(const Position& pos, ExtMove* mlist)
{
	...
	assert(Type == CAPTURES || Type == NON_EVASIONS || Type == QUIETS);
...
ExtMove* generate<LEGAL>(const Position& pos, ExtMove* mlist) {
	return pos.in_check() ? generate<EVASIONS>(pos, mlist)
