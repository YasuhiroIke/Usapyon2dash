...
// NONE
#define	PIECENUMBER_NONE	0
// 玉（１，２）を除く
#define MIN_PIECENUMBER	3
// 最大駒番号＝駒の数
...
#if defined(NANOHA)
const Value PawnValueMg   = Value(90+90);
...
#ifdef EVAL_DIFF
	extern const int KppIndex0[32];    // pieceをkppのindexに変換
	extern const int KppIndex1[32];    // pieceをkppのindexに変換
	extern const int HandIndex0[32];   // kpp_handのindexに変換
	extern const int HandIndex1[32];   // kpp_handのindexに変換
#endif // EVAL_DIFF
}
